...
public class MusicModule extends ReactContextBaseJavaModule implements ServiceConnection {
    ...
    public void enableRepeat() {
        waitForConnection(() -> {
            binder.getPlayback().onRepeatModeChanged(1);
        });
    }
    ...
    public void disableRepeat() {
        waitForConnection(() -> {
            binder.getPlayback().onRepeatModeChanged(0);
        });
    }

    @ReactMethod
    public void enableAutoplayUpNext() {
        waitForConnection(() -> {
            binder.autoPlayNext = true;
        });
    }

    @ReactMethod
    public void disableAutoplayUpNext() {
        waitForConnection(() -> {
            binder.autoPlayNext = false;
        });
    }
